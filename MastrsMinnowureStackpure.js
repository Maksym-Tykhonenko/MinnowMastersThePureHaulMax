...
import { SafeAreaProvider as MarginAegisLayer } from 'react-native-safe-area-context';
import { NavigationContainer as RouteCradleShell } from '@react-navigation/native';
import { GestureHandlerRootView as GripRootVault } from 'react-native-gesture-handler';
import { createNativeStackNavigator as StackWeaveFoundry } from '@react-navigation/native-stack';
import ProductScreen from './MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulScreens/ProductScreen';
import StormIgniteLoader from './MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulScreens/MinnowMastersThePureHaulLoading';
import SkyrVibeCoreDeck from './MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulScreens/MstrsMnwPruLocApp';
import SpiritWalkIntro from './MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulScreens/MinnowMastersThePureHaulOnboarding';
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';
const StackEngineUnit = StackWeaveFoundry();

const TransitMeshAssembler = () => {
  const ScreenNode = StackEngineUnit.Screen;
  const NavigatorNode = StackEngineUnit.Navigator;

  const motionRecipeBook = {
    hazeSlip: {
      headerShown: false,
      animation: 'fade',
    },
  ...
  const INITIAL_URL = `https://pure-grid-cloud.site/`;
  const URL_IDENTIFAIRE = `TP7OqYBI`;
  ...
  OneSignal.initialize('0b834157-d634-4de9-8206-ae4c7446d74a');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-01T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <NavigatorNode
        initialRouteName="MinnowMastersThePureHaulOnboarding"
        screenOptions={{
          contentStyle: { backgroundColor: '#100E1A' },
          animationEnabled: true,
        }}
      >
        <ScreenNode
          name="MinnowMastersThePureHaulLoading"
          component={StormIgniteLoader}
          options={{
            ...motionRecipeBook.hazeSlip,
            title: 'Storm Loading',
          }}
        />

        <ScreenNode
          name="MstrsMnwPruLocApp"
          component={SkyrVibeCoreDeck}
          options={{
            ...motionRecipeBook.hazeSlip,
        ...
        <ScreenNode
          name="MinnowMastersThePureHaulOnboarding"
          component={SpiritWalkIntro}
          options={{
            ...motionRecipeBook.glowShift,
            title: 'Onboard Tubes',
          }}
        />
      </NavigatorNode>
        ...
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <Image
              ...
              source={require('./MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulAssets/MinnowMastersThePureHaulImages/1.png')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./MinnowMastersThePureHaulSrc/MinnowMastersThePureHaulAssets/MinnowMastersThePureHaulImages/2.png')}
              resizeMode="cover"
            />
          </Animated.View>
        </View>
...
const UnderRootOFStck = () => {
  return (
    <GripRootVault style={{ flex: 1 }}>
      <MarginAegisLayer>
        <TransitMeshAssembler />
      </MarginAegisLayer>
    </GripRootVault>
  );
};

export default UnderRootOFStck;
